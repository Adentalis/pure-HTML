...
var FinancialData = [
  ...
  {
    date: new Date("01/10/2020"),
    product: "Book - How to become a Web Developer",
  ...
  },
  {
    ...
    product: "Flat",
    "product category": "Rental Income",
    amount: 120,
  ...
  {
    date: new Date("01/01/2020"),
    product: "How to become a Web Developer",
    "product category": "Books",
    amount: -35,
    "payment method": "Credit Card",
  },
...
var bgC = {
  ...
  violett: "rgba(153, 102, 255, 0.7)",
  grey: "rgba(201, 203, 207, 0.7)",
};
var backgroundColorCategory = [
  bgC.red,
  bgC.orange,
  bgC.yellow,
  bgC.green,
...
function accumulate(array) {
  return array.map(
    function (val) {
      return { x: val.x, y: (this.acc += val.y) };
    },
    { acc: balance }
  );
}
...
var income = FinancialData.filter(function (entry) {
  return entry.amount >= 0;
});
var cost = FinancialData.filter(function (entry) {
  return entry.amount < 0;
});

// Calculate overal totals, splitted for income and expenses, to determine relative weights
var incomeTotals = income.reduce(function (val, data) {
  return val + data.amount;
}, 0);
var costTotals = cost.reduce(function (val, data) {
  return val + data.amount;
}, 0);
...
//  -- category
var incomeGroupedByCategory = {
  category: groupByCategory(income).map((a) => a.x),
  amount: groupByCategory(income).map((a) => a.y),
};
var costGroupedByCategory = {
  category: groupByCategory(cost).map((a) => a.x),
  amount: groupByCategory(cost).map((a) => a.y),
};
...
groupByCategory(cost).forEach((elem, i) => {
  costGroupedByCategoryBenchmark.push({
    label: elem.x,
    backgroundColor: backgroundColorCategory[i],
    data: [
      {
        x: elem.y,
...
//  -- payment methos
var incomeGroupedByPaymentMethod = {
  category: groupByPaymentMethod(income).map((a) => a.x),
  amount: groupByPaymentMethod(income).map((a) => (a.y / incomeTotals) * 100),
};
var costGroupedByPaymentMethod = {
  category: groupByPaymentMethod(cost).map((a) => a.x),
  amount: groupByPaymentMethod(cost).map((a) => (a.y / costTotals) * 100),
};
